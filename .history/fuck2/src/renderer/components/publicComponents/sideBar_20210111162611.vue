<template>
  ...
  <!-- 左侧_导航栏 声明式导航 -->
    <div id='left'>
      ...
      <i v-for='(data,index) in side_list' :key="data.id" @click='selct_is_me(data.id)' :class='data.class' :class='active_class'></i>
...
export default {
  ...
  methods: {
    ...
    active_class(i) {
      ...
      return [this.current_id === i ? 'active_class' : ''] // 如果点击，则为当前元素添加激活样式
    }
  },
  mounted: function () {
    this.update_bg_urls()
