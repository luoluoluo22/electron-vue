<template>
  ...
  <div >
    <!-- 左侧_模板选择 -->
      <ul id="l_template better_scroll" >
        <li  class="template_banner" v-for='item in template_banner_src'>
            <img :src='item'>
        </li>
      </ul>
    </div>
...
export default {
  ...
  mounted: function () {
    ...
    // 当其他dom渲染完成之后执行
    this.$nextTick(() => {
      this.scroll = new BScroll('.better_scroll'{})
      this.fscroll = new BScroll('.flist', {
        scrollY: true,
        probeType: 3
      })
    })
