<template>
  ...
  <!-- 左侧_导航栏 声明式导航 -->
    <div id='left'>
      ...
      <i v-for='(data,index) in side_list' :key="data.id" @click='selct_is_me($event)' :class='side_list[1].class'></i>
  ...
  data () {
    return {
      ...
      side_list: [{
        ...
        class: 'icon-template-library icon'
      },
      {
        id: 2,
        name: 'background',
      ...
      },
      {
        id: 3,
        name: 'ai247',
        class: 'icon-ai247 icon'
      },
      {
        id: 4,
        name: 'seban',
        class: 'icon-seban icon'
      }
      ]
    }
  },
