...
export default {
  ...
  methods: {
    ...
    update_bg_urls: async function() {
      this.$options.data.template_banner_src = await this.$options.methods.get_bg_url();// 获取test()函数的返回值
      console.log(`已执行更新背景`)
    }
  },
  moted: function () {
    // eslint-disable-next-line no-unused-vars
    // this.update_bg_urls()un
    // this.$options.methods.update_bg_urls()
    console.log('已执行上方函数', this.$options.data())
  }
}
</script>
